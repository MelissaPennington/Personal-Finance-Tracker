...
class CSV:
    ...
    BUDGET_CSV_FILE = "budget_data.csv"  # Added budget file
    ...
    @classmethod
    def check_budget(cls, df):
        try:
            budget_df = pd.read_csv(cls.BUDGET_CSV_FILE)
            for category in df['category'].unique():
                actual_expense = df[df['category'] == category]['amount'].sum()
                budget_row = budget_df[budget_df['category'] == category]
                if not budget_row.empty:
                    budget_limit = budget_row['monthly_budget'].values[0]
                    if actual_expense > budget_limit:
                        print(f"Warning: {category} spending (${actual_expense:.2f}) exceeds the budget (${budget_limit:.2f})!")
                    else:
                        print(f"{category} spending (${actual_expense:.2f}) is within the budget (${budget_limit:.2f}).")
                else:
                    print(f"No budget set for category: {category}.")
        except FileNotFoundError:
            print(f"Budget file {cls.BUDGET_CSV_FILE} not found. Please create it to use budget tracking.")
        except Exception as e:
            print(f"An error occurred during budget check: {e}")
    ...
    def get_transactions(cls, start_date, end_date):
        try:
            ...
            # Check budget here
            cls.check_budget(filtered_df)  # Added budget check
